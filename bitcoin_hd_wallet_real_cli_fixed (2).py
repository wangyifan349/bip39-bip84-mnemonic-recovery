...
- Forces 24 words when importing mnemonics and validates the BIP39 checksum.
- Automatically derives and clearly prints BIP84 bc1q, BIP86 bc1p, BIP44 1, and BIP49 3 addresses.
- Supports standard paths and hardened change/index variants.
- Supports private-key import: WIF, hexadecimal, 0x hexadecimal, decimal, and hex:/dec: prefixes.
- Runs a self-test at startup; stops immediately on failure to prevent use of incorrect addresses.
- Prints only to stdout and does not write export files.

Dependency:
    pip install ecdsa

Security reminders:
- For real funds, run only in an offline, clean environment.
- Terminal output contains the mnemonic, seed, private keys, WIF, and xprv; do not run on an online machine or shared terminal.
"""
...
def ask_int(prompt: str, default: int, minimum: int = 0, maximum: int | None = None) -> int:
    while True:
        ...
        try:
            ...
            continue
        if n < minimum:
            print(f"Cannot be less than {minimum}.")
            continue
        if maximum is not None and n > maximum:
            print(f"Cannot be greater than {maximum}.")
            continue
        return n
...
def ask_passphrase_once() -> str:
    try:
        return getpass.getpass("BIP39 passphrase; press Enter for none: ")
...
def print_mnemonic_report(validation: MnemonicValidation, seed: bytes, passphrase: str, generated_entropy_hex: str | None) -> None:
    print_header("BIP39 24-WORD MNEMONIC")
...
def print_first_addresses(master: BIP32PrivateNode, network: Network, account: int, change: int) -> None:
    print_header("FIRST STANDARD RECEIVING ADDRESS; bc1 ADDRESSES ARE SHOWN FIRST")
    ...
    print("-" * 100)
    print("Confirmation: BIP84 should show bc1q...; BIP86 should show bc1p... .")
...
def print_hd_wallet(
    ...
    print_header("HD CONFIG / AUTOMATIC DERIVATION CONFIGURATION")
    ...
    print("\nNote: standard is the most wallet-compatible path; the variants with hardened change/index are BIP32-valid but non-standard for watch-only usage.")

    print_first_addresses(master, network, account, change)

    print_header("ADDRESS SUMMARY")
    ...
    print_header("FULL ADDRESS DETAILS")
    for variant, harden_change, harden_index in DERIVATION_VARIANTS:
        for index in range(count):
            for purpose in PRINT_ORDER:
                rec = make_address_record(master, network, purpose, account, change, index, variant, harden_change, harden_index)
    ...
    print_header("DONE")
    print("All addresses are complete: BIP39 checksum, BIP32 derivation, private key/WIF, public key, address encoding, Base58Check/Bech32/Bech32m, and xpub/xprv cross-checks.")


def print_single_private_key_report(imported: ImportedPrivateKey) -> None:
    privkey = imported.privkey
    print_header("IMPORTED PRIVATE KEY")
    ...
    for network in ("mainnet", "testnet"):
        print_header(f"SINGLE-KEY ADDRESSES ON {network.upper()} / NON-HD SINGLE-PRIVATE-KEY ADDRESSES")
        for purpose in PRINT_ORDER:
            info = PURPOSE_INFO[purpose]
...
def print_intro() -> None:
    ...
    ================================================================================================
    Offline Bitcoin Wallet CLI / Offline Bitcoin Receiving Address Tool
    ================================================================================================
    Minimal default inputs:
      - Generate mode always creates 24 BIP39 words.
      - Asks once for the BIP39 passphrase; press Enter for none.
      - Automatically prints BIP84 bc1q, BIP86 bc1p, BIP44 1, and BIP49 3.
      - Automatically includes four path variants: standard, index-hardened, change-hardened, and change-and-index-hardened.
      - Prints only; does not export files.
    ================================================================================================
    """).strip())
...
def menu() -> str:
    print("\nPlease choose:")
    print("  1) Generate a new 24-word BIP39 mnemonic and automatically print many HD receiving addresses")
    print("  2) Import an existing 24-word BIP39 mnemonic and automatically print many HD receiving addresses")
    print("  3) Import a private key/WIF and show decimal, hexadecimal, WIF, and BIP44/49/84/86 addresses")
    print("  4) Run the full self-test again")
    print("  5) Exit")
    return ask_line("Enter option; press Enter for 1", "1")


def run_generate_flow() -> None:
    words = get_wordlist()
    passphrase = ask_passphrase_once()
    count = ask_int("How many indexes to print for each BIP/path variant; press Enter for 20", 20, minimum=1)
...
def run_import_mnemonic_flow() -> None:
    print("Paste 24 English BIP39 words. Exactly 24 words are required, and the checksum must be valid.")
...
def run_import_private_key_flow() -> None:
    print("Supported formats: WIF, 64-character hexadecimal, 0x hexadecimal, decimal, hex:..., dec:...")
...
def interactive_main() -> None:
    ...
    print_intro()
    print("Running startup self-test; if it fails, the script will stop and will not generate addresses.")
    ...
    while True:
        ...
        try:
            ...
            print("\nERROR:", exc)
            print("The current operation has stopped; address generation did not continue.")
